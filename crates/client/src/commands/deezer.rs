...
#[macros::command(description = "Download a song from Deezer (tries to upload it directly to the chat)")]
...
async fn deezer(ctx: &Context<'_>, query: &str, with_banner: bool) -> Result<()> {
  ...
  tracing::info!("converting query to url…");
  let url = query_to_url(ctx, query).await?;
  tracing::debug!(%url, "selected");

  tracing::info!("selecting track quality…");
...
async fn query_to_url(ctx: &Context<'_>, query: &str) -> Result<String> {
  ...
  match Url::parse(&query) {
    ...
    Err(_) => {
      ...
      tracing::info!("selecting track url…");
...
fn render(path: impl AsRef<OsStr>) -> Result<cairo::ImageSurface> {
  ...
  {
    let png = ffmpeg::album_cover(&path)?;
    let cover = cairo::ImageSurface::create_from_png(&mut Cursor::new(png))?;

    cc.save()?;
    cc.translate(IMAGE_W as f64 / 2.0, IMAGE_H as f64 / 2.0);
    cc.scale1(IMAGE_W as f64 / cover.width() as f64);
