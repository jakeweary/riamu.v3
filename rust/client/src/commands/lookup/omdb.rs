...
pub async fn run(ctx: &Context<'_>, movie: &str) -> Result<()> {
  ...
  let json = api.query(movie).await?;

  let omdb::api::Response::Success(json) = json else {
...
mod omdb {
  ...
  impl<'a> Api<'a> {
    ...
    pub async fn query(&self, title: &str) -> reqwest::Result<api::Response> {
  ...
  pub mod api {
    ...
    impl Success {
      ...
      pub fn people(&self) -> Result<String, std::fmt::Error> {
      ...
      pub fn details(&self) -> Result<String, std::fmt::Error> {
        let mut acc = String::new();
      ...
      pub fn ratings(&self) -> Result<String, std::fmt::Error> {
        let mut acc = String::new();
